...
public final class GoogleCalendarConverters {
     ...
     * It is assumed that the event is created from the supplied activity
     *
     * @param event    A non-null event which was generated from the supplied Activity and converted by the
     ...
     */
    public static Event getUpdatedEvent(final Event event, final Activity activity) {

        // Check sanity
        Validate.notNull(event, "Cannot handle null event argument.");
        Validate.notNull(activity, "Cannot handle null activity argument.");

        final long eventId = getActivityID(event);
        Validate.isTrue(eventId == activity.getId(),
                "To update an event from an activity, they must have the same ID. Got event ID ["
                        + eventId + "] and activity ID [" + activity.getId() + "]"
        );
        ...
        checkEqualityAndUpdateIfRequired(updated, "summary", event, comparison);
        checkEqualityAndUpdateIfRequired(updated, "description", event, comparison);
        checkEqualityAndUpdateIfRequired(updated, "location", event, comparison);
        checkEqualityAndUpdateIfRequired(updated, "start", event, comparison);
        checkEqualityAndUpdateIfRequired(updated, "end", event, comparison);

        // Check the relevant Maps.
        final SortedMap<String, EventAttendee> originalAttendees = sortAttendees(event);
        ...
        if (overwriteAttendees) {
            event.setAttendees(comparison.getAttendees());
            updated[0] = true;
        }
        ...
        if (overwritePrivates) {
            event.getExtendedProperties().setPrivate(comparison.getExtendedProperties().getPrivate());
            updated[0] = true;
        }
     ...
     *
     * @param event A non-null event which was generated from an Activity and converted by the {@code
     *              GoogleConverter#convert()} method (implying it has a property with the name
     *              {@code ACTIVITY_ID_PROPERTY} in its private extended properties holding the activity ID).
     * @return The ID for the Activity which created the supplied Event.
     * @throws NullPointerException if either the event, its extendedProperties or its private properties was null.
     */
    @SuppressWarnings("all")
    public static long getActivityID(final Event event) throws NullPointerException {
        ...
        if (extendedProperties == null) {
            throw new IllegalArgumentException("The ExtendedProperties Map of the supplied event was null. "
                    + "The event is not generated from an intranet Activity.");
        }

        // Find the private properties of the Event, and retrieve its activityID
        final Map<String, String> privateProperties = extendedProperties.getPrivate();
        if (privateProperties == null) {
            throw new IllegalArgumentException("The PrivateProperties Map of the supplied event was null. "
                    + "The event is not generated from an intranet Activity.");
        }

        // All done.
        return Integer.parseInt(privateProperties.get(ACTIVITY_ID_PROPERTY));
    }
    ...
    /**
     * <p>Creates the following (nonstandard) Reminders for the supplied Event:</p>
     * <ol>
